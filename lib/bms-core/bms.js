...
const BMS = {
    ...
    writeValueAsync: function(id, value, opts) {
        ...
        if (bp[id]) {
            // La POLITIQUE est appliquée ici, au niveau BMS : droits d'accès,
            // bornes, cadence. Le pilote ne fait que le transport. Laisser un
            // pilote décider de ces règles reviendrait à les réécrire pour
            // chaque protocole, et à les perdre au premier pilote qui oublie.
            if (bp[id].access !== 'read_write') {
                return Promise.resolve({ ok: false, error: 'read-only point: ' + id });
            }
            let v = value;
            if (bp[id].min !== undefined && v < bp[id].min) v = bp[id].min;
            if (bp[id].max !== undefined && v > bp[id].max) v = bp[id].max;

            const source = (opts && opts.source) || 'rule';
            const verdict = safety.allowChange(id, v, bp[id].value);
            if (!verdict.allowed) {
                if (node && node.warn) node.warn('⚠ ' + verdict.reason);
                safety.record(id, v, source, { ok: false, error: verdict.reason });
                return Promise.resolve({ ok: false, error: verdict.reason });
            }
            const changed = bp[id].value !== v;

            if (!io) return Promise.resolve({ ok: this.writeValue(id, v) });
            return io.write(id, v, opts || {}).then((res) => {
                // Journaliser ici : c'est le seul endroit que traversent TOUTES
                // les écritures asynchrones. L'oublier rendait le journal des
                // commandes aveugle aux écritures réussies.
                if (changed || (res && !res.ok)) {
                    safety.record(id, v, source, res);
                }
                return res;
            });
        }
