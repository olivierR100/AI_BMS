...
function createSafety(ctx) {
    ...
    return {
        ...
        realPointsCommandedBy(cfg) {
            ...
            // Le portail protège du MATÉRIEL RÉEL, pas d'un simulateur. En mode
            // « simulated » le pilote s'appelle aussi « bacnet » : s'en tenir au
            // nom du pilote ferait demander une approbation pour chaque
            // configuration appliquée sur le banc de test.
            if (global.get('bacnetMode') !== 'real') return [];
            ...
            return [...targets].filter((id) => {
                // Un état interne (st_*) ne quitte jamais le BMS : il n'a pas de
                // pilote et ne doit pas déclencher le portail.
                if (!bacnetPoints[id]) return false;
