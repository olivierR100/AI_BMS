...
/**
 * Couples actionneur/capteur physiquement liés à l'intérieur d'une zone : agir
 * sur le premier déplace la mesure du second. Conditionner l'un sur l'autre
 * ferme une boucle par le bâtiment, pas par le logiciel.
 */
const COUPLED_PAIRS = [
    { actuator: 'lighting',    sensor: 'light' },        // la lampe éclaire le luxmètre
    { actuator: 'ventilation', sensor: 'co2' },          // la ventilation évacue le CO2 mesuré
    { actuator: 'hvac_temp',   sensor: 'temperature' },  // la consigne déplace la température
];
...
function analyseConfig(cfg, context) {
    ...
    for (const group of groups) {
        ...
        for (const rule of group.rules || []) {
            ...
            // ── 2. Rétroaction physique actionneur → capteur ─────────────────
            //
            // Une lampe éclaire le capteur de luminosité de sa propre zone ; une
            // ventilation fait baisser le CO2 qu'elle mesure ; un chauffage
            // change la température qui le commande. Conditionner l'actionneur
            // sur ce capteur ferme une boucle par le monde physique.
            //
            // Trouvé sur ce jeu de règles : 11 règles « Actif+Sombre → lampe
            // allumée » conditionnées par le lux de la zone qu'elles éclairent.
            // Aujourd'hui la boucle ne bat pas — rien n'éteint sur « clair » —
            // mais elle rend la règle non réentrante : lampe éteinte à la main
            // alors que la zone est occupée, elle ne se rallumera pas tant que
            // le lux n'aura pas rebaissé. Et le jour où quelqu'un ajoute la
            // règle symétrique, l'oscillation est garantie.
            for (const target of targets) {
                const tMeta = metadata[target];
                if (!tMeta || !tMeta.zone) continue;
                const actuatorTag = COUPLED_PAIRS.find((p) => (tMeta.tags || []).includes(p.actuator));
                if (!actuatorTag) continue;

                for (const fact of conditionFacts) {
                    const fMeta = metadata[fact];
                    if (!fMeta || fMeta.zone !== tMeta.zone) continue;
                    if (!(fMeta.tags || []).includes('sensor')) continue;
                    if (!(fMeta.tags || []).includes(actuatorTag.sensor)) continue;

                    add({
                        severity: 'medium', code: 'actuator-sensor-feedback', ...where,
                        message: `« ${rule.name} » commande « ${target} » en se conditionnant sur ` +
                                 `« ${fact} », capteur de la même zone que cet actionneur influence ` +
                                 `physiquement. La boucle se referme par le bâtiment.`,
                        hint: `Prévoir une hystérésis explicite : deux seuils distincts (allumer sous ` +
                              `X, éteindre au-dessus de Y avec Y nettement supérieur à X), ou mémoriser ` +
                              `la décision dans un état plutôt que de relire la grandeur influencée. ` +
                              `Sans cela la règle n'est pas réentrante, et la règle inverse la ferait osciller.`,
                    });
                    break;
                }
            }
