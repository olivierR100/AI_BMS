...
/*
 * Capture du client BACnet que BDDevice construit pour lui-même.
 *
 * Pourquoi : la bibliothèque garde son client dans un champ privé, sans
 * accesseur, et y branche un refus pour SubscribeCOVProperty. Or ce service est
 * la façon normalisée de régler un incrément par abonnement, et le simulateur
 * doit l'accepter (cf. `cov-property.js`). Il faut donc atteindre ce client pour
 * remplacer son écouteur.
 *
 * Comment : `device.js` fait `const { default: BACnetClient } = bacnet;` au
 * chargement du module. En remplaçant `.default` du module client AVANT de
 * charger le module device, on lui fait construire une sous-classe qui se
 * signale. La substitution est défaite aussitôt après.
 *
 * Portée : ce processus seulement — le simulateur. Le BMS ne charge jamais ce
 * fichier, et le comportement observé sur le réseau reste du BACnet standard.
 */
const bacnetClientModule = require('@bacnet-js/client');
const RealBacnetClient = bacnetClientModule.default;
let capturedClient = null;
class CapturingBacnetClient extends RealBacnetClient {
    constructor(opts) { super(opts); capturedClient = this; }
}
bacnetClientModule.default = CapturingBacnetClient;
const { BDDevice, BDAnalogInput, BDAnalogValue, BDBinaryValue } = require('@bacnet-js/device');
bacnetClientModule.default = RealBacnetClient;

const { EngineeringUnits, BinaryPV, PropertyIdentifier, ApplicationTag,
        ErrorClass, ErrorCode, ServicesSupported, ServicesSupportedBitString } = bacnetClientModule;

const installPoints = require('../bms-core/points');
const { UNIT_DEFAULTS } = require('../bms-core/cov');
const { installCovProperty } = require('./cov-property');
...
function parseArgs(argv) {
    const opts = { port: 47810, deviceId: 1234, interface: '0.0.0.0', tick: 2000,
                   controlPort: 47811, quiet: false, covDeadband: true, covProperty: true };
    for (let i = 2; i < argv.length; i++) {
        ...
        switch (k) {
            ...
            // Bande morte des OBJETS (voie SubscribeCOV simple, et repli quand une
            // requête ne porte pas d'incrément).
            case '--no-cov-deadband': opts.covDeadband = false; break;
            // Se comporter comme un automate qui n'implémente pas
            // SubscribeCOVProperty : le service n'est ni annoncé ni traité. C'est
            // ainsi qu'on vérifie que le BMS masque bien ses réglages d'incrément
            // au lieu d'afficher des commandes sans effet.
            case '--no-cov-property': opts.covProperty = false; break;
            case '--help':
                console.log('usage: node server.js [--port 47810] [--device-id 1234] ' +
                            '[--interface 0.0.0.0] [--tick 2000] [--control-port 47811] ' +
                            '[--no-cov-deadband] [--no-cov-property] [--quiet]');
                process.exit(0);
                break;
 ...
 *
 * `COV_Increment` est posé dès la construction, aux incréments par unité de
 * `bms-core/cov.js` : le simulateur lancé seul doit déjà être silencieux. La
 * propriété reste inscriptible, et c'est le BMS qui y écrit ses profils une fois
 * connecté — comme il le ferait sur un automate réel.
 *
 * Un objet binaire n'a pas de `COV_Increment` : toute transition notifie.
 */
function buildObjects(device, bacnetPoints, { covDeadband = true } = {}) {
    ...
    for (const [factId, p] of Object.entries(bacnetPoints)) {
        ...
        if (isBool) {
            ...
            const opts = {
                ...
                covIncrement: covDeadband ? (UNIT_DEFAULTS[p.units] ?? 0) : 0,
            };
...
function start(opts) {
    ...
    const objects = buildObjects(device, bacnetPoints, { covDeadband: opts.covDeadband });

    const say = (...a) => { if (!opts.quiet) console.log(...a); };

    /*
     * SubscribeCOVProperty : l'incrément par abonnement, que la bibliothèque
     * refuse et que ce simulateur doit savoir traiter. Le service est annoncé
     * dans Protocol_Services_Supported, ce que le BMS lit pour décider s'il
     * propose ou masque les réglages d'incrément.
     */
    let covProperty = null;
    if (capturedClient && opts.covProperty) {
        covProperty = installCovProperty({
            client: capturedClient, device, deviceId: opts.deviceId, objects,
            constants: { PropertyIdentifier, ApplicationTag, BinaryPV, ErrorClass, ErrorCode,
                         ServicesSupported, ServicesSupportedBitString },
            log: (m) => say('  ' + m),
        });
        // Toute évolution de valeur passe par ici, d'où qu'elle vienne (physique,
        // écriture réseau, canal de contrôle) : les abonnements par propriété
        // sont évalués au même endroit.
        for (const [factId, entry] of objects) {
            entry.object.presentValue.addCoVListener(() => {
                const raw = entry.object.presentValue.getValue();
                covProperty.publish(factId, entry.isBool ? (raw === BinaryPV.ACTIVE) : round1(raw));
            });
        }
    }
    ...
    say(opts.covDeadband
        ? `  bande morte COV par unité : ${Object.entries(UNIT_DEFAULTS)
              .filter(([u]) => u !== 'bool').map(([u, v]) => `${v} ${u}`).join(' · ')}`
        : `  bande morte COV DÉSACTIVÉE — chaque frémissement sera notifié`);
    say(covProperty
        ? `  SubscribeCOVProperty : supporté (incrément par abonnement)`
        : !opts.covProperty
            ? `  SubscribeCOVProperty : désactivé (--no-cov-property) — comme un automate qui l'ignore`
            : `  SubscribeCOVProperty : INDISPONIBLE — client BDDevice non capturé`);
    ...
    const tick = () => {
        ...
        try {
            ...
            // 4. Faire le ménage dans les abonnements expirés — un client parti
            //    sans se désabonner ne doit pas rester servi indéfiniment.
            if (covProperty && ticks % 15 === 0) covProperty.sweepExpired();

            if (!opts.quiet && ticks % 30 === 0) {
                const cov = covProperty ? covProperty.stats() : null;
                say(`  tick ${ticks} · ${result.changes} changements · ${result.zoneCount} zones ` +
                    `· ${result.outsideTemp.toFixed(1)}°C dehors` +
                    (cov ? ` · ${cov.active} abonnements COV-propriété, ${cov.sent} notifications` : ''));
            }
        } catch (e) {
    ...
    const control = http.createServer((req, res) => {
        ...
        // Vue sur les abonnements SubscribeCOVProperty : ce que le serveur croit
        // devoir à chaque client, incrément et référence compris. Sans cela, un
        // désaccord entre les deux côtés est indiagnosticable.
        if (req.method === 'GET' && req.url.startsWith('/covsubs')) {
            if (!covProperty) return send(200, { supported: false, subscriptions: [] });
            return send(200, {
                supported: true,
                stats: covProperty.stats(),
                subscriptions: [...covProperty.subscriptions.values()].map((s) => ({
                    point: s.factId, pid: s.pid, increment: s.increment,
                    confirmed: s.confirmed, reference: s.reference,
                    lifetime: s.lifetime,
                    expiresInMs: s.expiresAt ? Math.max(0, s.expiresAt - Date.now()) : null,
                })),
            });
        }
        if (req.method !== 'POST' || !req.url.startsWith('/force')) {
            return send(404, { error: 'POST /force {id,value} · GET /state · GET /covsubs' });
        }
        let raw = '';
    ...
    control.listen(opts.controlPort, opts.interface === '0.0.0.0' ? undefined : opts.interface, () => {
        ...
        say(`  abonnements COV-propriété : http://${opts.interface}:${opts.controlPort}/covsubs`);
    });
    ...
    return { device, objects, bacnetPoints, covProperty, stop: shutdown };
}
